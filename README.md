...
`vta_api_key` 的固定顺序兜底。迁移 API 不会在普通读取时自动调用；任何成功写入
canonical 或显式清除，且浏览器存储可写时，都会写入迁移标记并清理旧别名，避免旧凭据
复活。浏览器存储不可用时仅在当前标签页内存中封存令牌，刷新页面后需要重新输入。
...
- 提交转录、查询私有任务/历史，以及转录页的重新校对、重新总结、生成详细笔记等
  ...
  它，不会再次弹窗；令牌缺失或首次收到 `401` 时才显示一次输入框，最多重放
  一次，`403/404/409` 和不确定的 POST 网络错误不会自动重试。
- 首页在“高级设置”中填写令牌；历史页的“清除鉴权”会撤销私有请求并清空当前页的
  私有数据；转录结果页提供“设置/更换访问令牌”和“清除访问令牌”。清除或更换会
  同步其他打开的标签页，清除后旧别名不会复活。访问令牌只应输入到可信浏览器，
  不要把它放进 URL、分享链接或日志。
